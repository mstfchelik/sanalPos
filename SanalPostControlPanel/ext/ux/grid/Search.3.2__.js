...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	...
	,onRender:function() {
		...
		// handle position
		if('right' === this.align) {
			 tb.insert(this.tbPosition, '->');  //tb.addFill(); ---
		}
		else {
			tb.insert(this.tbPosition, '-');	/*if(0 < tb.items.getCount()) {   ---
													  tb.addSeparator();
												  }*/
		}
		this.tbPosition++;
		// add menu button
		tb.insert(this.tbPosition, {            //  tb.add({  ---
		...
		this.field.on('render', function() {
		  ...
		  tb.insert(this.tbPosition, this.field);    //tb.add(this.field);
	...
	,onTriggerSearch:function() {
		...
		// ask server to filter records
			...
			// reload store
			store.proxy.getConnection().abort();  // 3.2 de abort olmuyordu
			store.reload();
		}

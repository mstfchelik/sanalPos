...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	triggerCount: 0,								// medet: 1 saniye gecikmeli arama için
	triggerDelay: 1500, 							// medet: 1 saniye gecikmeli arama için
	searchText : 'Search',
	...
	onKeyUp : function() {
		...
		if (0 === length || this.minChars <= length) {
			caller = this;										// medet: 1 saniye gecikmeli arama için
			this.triggerCount++;
			// medet: 1 saniye gecikmeli arama için
			this.grid.store.proxy.getConnection().abort(); // 3.2 de önceki isteği iptal etmesi için
			setTimeout("caller.onTriggerSearch()",this.triggerDelay);// medet: 1 saniye gecikmeli arama için
			//this.onTriggerSearch(); 							// kapatıldı  1 saniye gecikmeli arama için 
		}
	},
	...
	onTriggerSearch : function() {
		if(this.triggerCount>1){  	// medet: 1 saniye gecikmeli arama için
			this.triggerCount--;
			return;
		}
		...
		if ('local' === this.mode) {
			...
			store.proxy.getConnection().abort(); // 3.2 de önceki isteği iptal etmesi için
			store.reload();
			this.triggerCount--;
		}
	},
